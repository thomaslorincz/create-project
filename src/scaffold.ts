...
export async function run(argv: string[]) {
	...
	const packageName =
		options.packageName ?? toPackageName(path.basename(targetDir));
...
async function prepareTargetDirectory({ force, targetDir }: ScaffoldOptions) {
	...
	if (!force) {
		throw new Error(
			`${targetDir} is not empty. Re-run with --force to overwrite it.`,
		);
	}
...
async function writeRootFiles(options: ScaffoldOptions) {
	...
	await writeFile(
		path.join(options.targetDir, ".vscode", "settings.json"),
		vscodeSettings,
	);
	await writeFile(
		path.join(options.targetDir, ".vscode", "extensions.json"),
		vscodeExtensions,
	);
}
...
async function scaffoldFrontend(options: ScaffoldOptions) {
	...
	await writeFile(
		path.join(frontendDir, ".oxlintrc.json"),
		frontendOxlintConfig,
	);
...
async function writeBackendFiles(options: ScaffoldOptions) {
	...
	await writeFile(
		path.join(backendDir, "src", "routers", "health.ts"),
		healthRouterTs,
	);
}
